...
public interface DatacoreApi {
   ...
   public String NATIVE_QUERY_COMMON_DOC = "'operator' (if any, else defaults to '=') can be in "
         ...
         + "error is raised, and a ranged query on an indexed required field should be used instead. "
         + "A ranged query is a query with a > or < comparison criteria on a sorted Resource (ex. "
         ...
         + "javascript:return findDataByType($(this).attr('href'));"
         + "\">/dc/type/dcmo:model_0?dc:modified=-</a>. Beware, if the field is not unique, "
         ...
         + "\n<br/>\n"
         + "Secondly, MongoDB <a href=\"http://docs.mongodb.org/v2.6/reference/operator/meta/maxScan/\">maxScan</a> "
         + "is set on the query to the maximal queried fields' "
   ...
   @ApiImplicitParams({
      ...
      @ApiImplicitParam(name=VIEW_HEADER, paramType="header", dataType="string",
            value="Examples : nothing (all), empty (@id, o:version), 'dc:DublinCore_0' (also "
