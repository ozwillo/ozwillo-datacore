...
public class EntityPermissionEvaluator implements PermissionEvaluator {
   ...
   public boolean hasPermission(Authentication authentication,
      ...
      DCModel model = entityService.getModel(dataEntity);
      if (model == null) {
      ...
      // TODO move it to user.isAdmin() ?
      //if (hasRole("admin") || hasRole("t_" + model.getName() + "_admin")) {
      if (isAdmin() || isModelTypeAdmin()) {
         return true;
      }
      ...
      case "read" :
         ...
         return entityService.getModel(dataEntity).getSecurity().isPublicRead()
               || hasAnyEntityAclGroup(dataEntity.getReaders());
      case "write" :
         ...
         return model.getSecurity().isPublicWrite()
               || hasAnyEntityAclGroup(dataEntity.getWriters())
               || hasAnyEntityAclGroup(dataEntity.getOwners());
      case "create" :
         // to be used in @PreAuthorize on update ????
         ///return entityService.getModel(dataEntity).getSecurity().hasCreator(user); /// TODO
         return true;
      case "changeRights" :
         // to be used in @PostAuthorize on GET
         // NB. readers have to contain ALSO writers & owners (i.e. be precomputed)
         // to allow efficient query filtering by rights criteria
         // => TODO that in PermissionAdminApi/Service
         return hasAnyEntityAclGroup(dataEntity.getOwners());
      case "admin" : // TODO any use ?? rather done in @PreAuthorize (?)
         ///return model.hasAdmin(user) || hasAdmin(user); /// TODO
         return false;
      default :
   ...
   private boolean isModelTypeAdmin() {
      // TODO Auto-generated method stub
      return false;
   }
    ...
    * @return
    */
   private boolean hasAnyEntityAclGroup(List<String> entityAclGroups) {
      if (entityAclGroups == null || entityAclGroups.isEmpty()) { // TODO null should not happen ?!?
         return false;
      }
      
      Set<String> currentUserEntityGroups = MockAuthenticationService.getCurrentUserEntityGroups();
